...
int main(int argc, char** argv) {
    ...
    CvScalar BLUE = cvScalar(0.f, 0.f, 1.0, 0.0);
    // Fill the image with the specified color (RGB: 200, 100, 50)
    cvSet(CLR, BLUE, NULL);

    cvNamedWindow("Mask32", CV_WINDOW_AUTOSIZE);
    cvShowImage("Mask32", Mask32); // Display 

    cvNamedWindow("CLR_IMG", CV_WINDOW_AUTOSIZE);
    cvShowImage("CLR_IMG", CLR); // Display 
    ...
    cvAddWeighted(im32, 1, ExtendMask, 0.8, 0, im32);
