...
void draw_label(IplImage* input_image, const char* label, int left, int top)
{
    ...
    // Top left corner.
    tlc = cvPoint(left, top - label_size.height - baseLine);
    // Bottom right corner.
    brc = cvPoint(left + label_size.width, top);
    ...
    cvPutText(input_image, label, cvPoint(left, top - baseLine), &font, WHITE);
}
...
int main(int argc, char** argv){
    ...
    CvPoint tlp = cvPoint(200, 200);
    CvPoint brp = cvPoint(350, 350);
